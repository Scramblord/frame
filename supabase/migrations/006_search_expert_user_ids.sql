-- Keyword search for consumer marketplace (matches any token against expert_profiles.keywords)
...
as $$
  ...
  inner join public.profiles pr on pr.user_id = ep.user_id and pr.role = 'expert'
  where
    p_search is null
    or trim(p_search) = ''
    or exists (
